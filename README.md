...
To grab your token, I recommend using the [discord-get-user-token](https://chromewebstore.google.com/detail/discord-get-user-token/accgjfooejbpdchkfpngkjjdekkcbnfd) Chrome extension. You can also follow other guides on Google if you are paranoid.

Just keep in mind that logging out of your account on the web kills that token. If you never log out, you are fine. But if you switch accounts often, I recommend logging in from a spare browser profile, copying the token from there, and just leaving that browser session alone.
